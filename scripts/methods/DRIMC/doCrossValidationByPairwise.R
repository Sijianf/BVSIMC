doCrossValidationByPairwise <- function(inMat, kfold = 10, numSplit = 5, seeds) {
  # INPUT
  # inMat: input matrix
  ...
  tripletpos <- triplet[value > 0]
  tripletneg <- triplet[value == 0]
  
  numTripos <- nrow(tripletpos)
  ...
  for (i in 1:numSplit) {
    #######
    foldspos <- getCvIndex(totNum = numTripos, nfold = kfold, seedval = seeds[i])
    #######
    for (j in 1:kfold) {
      
      currIndexpos <- foldspos[[j]]
      
      testDatapos <- tripletpos[currIndexpos]
      
      testData <- rbind(testDatapos, tripletneg)
      ...
      tmpTripletpos <- tripletpos
      tmpTripletpos[currIndexpos, "value"] <- 0
      tmpTriplet <- rbind(tmpTripletpos, tripletneg)
