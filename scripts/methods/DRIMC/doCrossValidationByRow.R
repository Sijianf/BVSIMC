doCrossValidationByRow <- function(inMat, kfold = 10, numSplit = 5, seeds) {
  # INPUT
  # inMat: input matrix
  # kfold: k-fold cross-validation
  # numSplit: number of splits
  # seeds: seed values for several splits 
  ...
  for (i in 1:numSplit) {
    #######
    folds <- getCvIndex(totNum = numRow, nfold = kfold, seedval = seeds[i])
    #######
    for (j in 1:kfold) {
      ...
      ########################
      isTestRow <- triplet[, rowIndex %in% currIndex]
      currIndex <- which(isTestRow)
      ########################
