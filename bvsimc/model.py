...
#          numerically equivalent to the original "c" variant; see
#          scripts/methods/BVSIMC/legacy/ for the archived original)
...
class BVSIMC:
    ...
    def p_star(self, vec, theta, lambda0, lambda1):
        # log-space spike/slab ratio (avoids underflow of the raw-density version)
    ...
    def update_theta(self, mat, alpha, beta):
        # vectorized count of rows with any nonzero entry
    ...
    def optimization(
        ...
        for iter_ in range(2, max_iter):
            ...
            # mu kept fixed at its initial value (not zeroed / not updated)
