...
# 使用parallel生成VCF (使用 -Q 0 & --ploidy 1)
parallel --halt now,fail=1 -a assemblies/all_accs.txt -j 10 --bar '
ACC={};
echo "=== START ${ACC} $(date) ===" >> logs/batch.log
...
minimap2 -ax asm5 reference/H37Rv.mmi assemblies/${ACC}.fna \
  | samtools sort -o assemblies/${ACC}.sorted.bam

samtools index assemblies/${ACC}.sorted.bam

# 2. mpileup + call（注意 -Q 0 & --ploidy 1）
bcftools mpileup -f reference/H37Rv.fna assemblies/${ACC}.sorted.bam \
  -a FORMAT/DP -Ou -Q 0 \
  | bcftools call -mv --ploidy 1 -Oz \
  -o assemblies/${ACC}.raw.vcf.gz
